...
def play_music():
    ...
    try:
        # Attempt to pick a song based on country and decade
        song_path, metadata = pick_song(country_code, decade)

        if song_path:
            # Store current song info in session for future use
            ...
            return jsonify({
                "status": "success",
                "music_files": [os.path.basename(song_path)],  # You could return more than one song if needed
...
def play(filename):
    # Attempt to fetch the song path from session first
    song_path = session.get("current_song")
    
    if not song_path or os.path.basename(song_path) != filename:
        # Fallback to search the song by filename if session is lost or the file doesn't match
...
def search_song_by_filename(filename):
    # Search for the song in your music archive directory
